'use client'

import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import { Badge } from '@/components/ui/badge'
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from '@/components/ui/accordion'
import { BookOpen, ChevronRight, CheckCircle2 } from 'lucide-react'
import { useNavigation } from '@/lib/store'
import { MODULES, PHASE_DESCRIPTIONS } from '@/lib/constants/modules'
import type { ModuleSlug } from '@/lib/types'
import { useState, useEffect } from 'react'

interface DocsListViewProps {
  onOpenDoc: (module: ModuleSlug, phase: number) => void
  readDocs: Record<ModuleSlug, Set<number>>
}

export function DocsListView({ onOpenDoc, readDocs }: DocsListViewProps) {
  const nav = useNavigation()
  const [openModules, setOpenModules] = useState<string[]>([])

  useEffect(() => {
    if (nav.currentModule && !openModules.includes(nav.currentModule)) {
      setOpenModules([nav.currentModule])
    }
  }, [nav.currentModule, openModules])

  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      <div className="mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold mb-2">Tài liệu học tập</h1>
        <p className="text-white/40 text-sm">Đọc tài liệu đầy đủ cho tất cả các giai đoạn. Học theo tốc độ của bạn!</p>
      </div>

      <Accordion type="multiple" value={openModules} onValueChange={setOpenModules} className="space-y-4">
        {(['systema', 'argos', 'cognos', 'ludus'] as ModuleSlug[]).map((slug) => {
          const mod = MODULES[slug]
          const moduleReadCount = readDocs[slug]?.size || 0

          return (
            <AccordionItem key={slug} value={slug} className="border border-white/10 rounded-xl bg-white/[0.02] overflow-hidden">
              <AccordionTrigger className="px-4 sm:px-6 py-4 hover:no-underline hover:bg-white/[0.02]">
                <div className="flex items-center gap-3 w-full">
                  <div className={`p-2 rounded-lg ${mod.accentBg}`}>
                    <span className={mod.color}>{mod.icon}</span>
                  </div>
                  <div className="flex-1 text-left">
                    <div className="flex items-center gap-2">
                      <CardTitle className="text-lg">{mod.name}</CardTitle>
                      <Badge variant="secondary" className={`text-xs ${mod.accentBg} ${mod.color} border-current`}>
                        {moduleReadCount}/5 đã đọc
                      </Badge>
                    </div>
                    <p className="text-white/40 text-sm mt-0.5">{mod.subtitle}</p>
                  </div>
                </div>
              </AccordionTrigger>
              <AccordionContent className="px-4 sm:px-6 pb-4">
                <div className="space-y-3 pt-2">
                  {mod.phases.map((phase) => {
                    const isRead = readDocs[slug]?.has(phase.phase) || false
                    const phaseDesc = PHASE_DESCRIPTIONS[slug]?.[phase.phase] || ''

                    return (
                      <Card
                        key={phase.phase}
                        className={`border transition-all cursor-pointer hover:border-cyan-500/30 ${
                          isRead ? 'border-green-500/20 bg-green-500/[0.02]' : 'border-white/10 bg-white/[0.01]'
                        }`}
                        onClick={() => onOpenDoc(slug, phase.phase)}
                      >
                        <CardContent className="p-4">
                          <div className="flex items-start gap-3">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-sm font-bold border ${
                              isRead
                                ? 'bg-green-500/20 border-green-500/40 text-green-400'
                                : 'bg-white/5 border-white/10 text-white/50'
                            }`}>
                              {isRead ? <CheckCircle2 className="w-4 h-4" /> : phase.phase}
                            </div>
                            <div className="flex-1 min-w-0">
                              <h4 className={`font-medium text-sm ${isRead ? 'text-green-400' : 'text-white/80'}`}>
                                {phase.title}: {phase.name}
                              </h4>
                              <p className="text-white/40 text-xs mt-1 line-clamp-2">{phaseDesc}</p>
                            </div>
                            <Button variant="ghost" size="sm" className={`shrink-0 ${mod.color}`}>
                              <BookOpen className="w-4 h-4 mr-1" />
                              Đọc
                              <ChevronRight className="w-3 h-3 ml-1" />
                            </Button>
                          </div>
                        </CardContent>
                      </Card>
                    )
                  })}
                </div>
              </AccordionContent>
            </AccordionItem>
          )
        })}
      </Accordion>
    </div>
  )
}
